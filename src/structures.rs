...
use crate::{utils::cross_product, A1, A2};

use super::utils::{get_angle, is_point_on_segment, project_onto_line, sort_by_norm};
...
impl VertexPoint {
    fn new(coordinate: A1, left: i8, right: i8) -> VertexPoint {
        VertexPoint { coordinate, left, right }
    }

    fn change_left(&mut self, new_left: i8) {
        self.left = new_left;
    }

    fn change_right(&mut self, new_right: i8) {
...
impl AnalyticTriangle {
    ...
    pub(crate) fn new(triangle: A2) -> AnalyticTriangle {
        ...
        let d1 = project_onto_line(&origin, &v1, &v2);
        let d2 = project_onto_line(&origin, &v2, &v3);
        let d3 = project_onto_line(&origin, &v3, &v1);
